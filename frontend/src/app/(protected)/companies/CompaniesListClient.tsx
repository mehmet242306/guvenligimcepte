...
import { fetchCompaniesFromSupabase, fetchArchivedFromSupabase, fetchDeletedFromSupabase, createCompanyInSupabase, archiveCompanyInSupabase, restoreCompanyInSupabase, deleteCompanyInSupabase } from "@/lib/supabase/company-api";
...
export function CompaniesListClient() {
  ...
  // Kalıcı silme kaldırıldı — silinen firma verileri AI öğrenme arşivine aktarılır
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-info/30 bg-info/5 px-4 py-3">
              <div className="flex items-center gap-2">
                <svg className="h-4 w-4 text-info shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" /></svg>
                <p className="text-sm font-medium text-foreground">Silinen firma verileri AI öğrenme arşivine aktarılmıştır. Risk analizi örüntüleri ve olay verileri Nova AI tarafından kullanılmaktadır.</p>
              </div>
            </div>
            <div className="grid gap-4 xl:grid-cols-2">{filtered.map(c => (
              <div key={c.id} className="rounded-xl border border-border bg-card p-4 shadow-[var(--shadow-soft)] opacity-75">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <span className="inline-flex items-center gap-1 rounded-lg border border-info/30 bg-info/10 px-2.5 py-1 text-[11px] font-medium text-info">
