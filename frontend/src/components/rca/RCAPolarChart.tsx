...
function polarColor(d: number, alpha = 0.55): string {
  ...
  if (d >= TAU_PRIMARY) return `rgba(216,90,48,${a})`;
  if (d >= TAU_SECONDARY) return `rgba(239,159,39,${a})`;
  return `rgba(180,178,169,${alpha})`;
}
  ...
  return (
    <Card aria-label={tr("polar.ariaLabel")}>
      <CardHeader className="pb-3">
        <CardTitle className="text-sm">{tr("polar.title")}</CardTitle>
      </CardHeader>
      <CardContent>
