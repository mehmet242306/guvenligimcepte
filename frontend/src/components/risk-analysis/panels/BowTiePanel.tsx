...
export function BowTiePanel({ bowTieValues, bowTieResult, onValuesChange }: BowTiePanelProps) {
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-2 text-xs">
          <span className="w-16 text-muted-foreground">Ham Risk</span>
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="w-16 text-muted-foreground">Artık Risk</span>
          <div className="flex-1 h-3 rounded-full bg-slate-200 dark:bg-neutral-800 overflow-hidden">
      ...
      <div className="space-y-2">
        {([
          { key: "threatProbability" as const, label: "Tehdit Olasılığı", max: 5 },
          { key: "consequenceSeverity" as const, label: "Sonuç Şiddeti", max: 5 },
          { key: "preventionBarriers" as const, label: "Önleyici Bariyer", max: 5 },
          { key: "mitigationBarriers" as const, label: "Azaltıcı Bariyer", max: 5 },
        ]).map(({ key, label, max }) => (
          <div key={key}>
