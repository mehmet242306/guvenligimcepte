...
export default function DigitalTwinPage() {
  ...
  const [sessionLinkedAssessmentId, setSessionLinkedAssessmentId] = useState<string | null>(null);
  ...
  // Load session detail
  useEffect(() => {
    if (!selectedSession) { setPoints([]); setStats(null); return; }
    (async () => {
  ...
  useEffect(() => {
    if (!selectedSession?.id) {
      setSessionLinkedAssessmentId(null);
      return;
    }
    let canceled = false;
    void (async () => {
      const aid = await fetchRiskAssessmentIdForScanSession(selectedSession.id);
      if (!canceled) setSessionLinkedAssessmentId(aid);
    })();
    return () => {
      canceled = true;
    };
  ...
  function riskAnalysisHrefForSession(): string {
    const cid = selectedSession?.companyId;
    if (!cid) return "/risk-analysis";
    if (sessionLinkedAssessmentId) {
      return `/risk-analysis?companyId=${encodeURIComponent(cid)}&loadId=${encodeURIComponent(sessionLinkedAssessmentId)}`;
    }
    return `/risk-analysis?companyId=${encodeURIComponent(cid)}`;
  }
          ...
          <div className="space-y-5">
            {!selectedSession ? (
              ...
              <>
                ...
                <div className="rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]">
                  ...
                  {stats && (
                    <div className="mt-4 space-y-3">
                      ...
                      {selectedSession?.companyId && stats.totalDetections > 0 ? (
                        <Link
                          href={riskAnalysisHrefForSession()}
                          className="inline-flex h-10 items-center rounded-xl border border-primary/30 bg-primary/10 px-4 text-xs font-semibold text-primary hover:bg-primary/15"
                        >
                          Bulguları Risk Analizi’nde aç →
                        </Link>
                      ) : null}
                    </div>
                  )}
                      ...
                      {selectedPoint ? (
                        <div className="rounded-xl border border-border bg-card p-4 shadow-[var(--shadow-soft)]">
                          ...
                          {selectedPoint.risksAtPoint.length > 0 ? (
                            <div className="mt-3 space-y-2 rounded-lg border border-border bg-secondary/10 p-3">
                  ...
                  <div className="rounded-2xl border border-border bg-slate-950 p-6 min-h-[600px] relative overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 rounded-lg bg-black/60 px-3 py-2 backdrop-blur-sm">
                      <p className="text-[10px] text-slate-400">
                        {points.length} nokta
                        {stats != null ? ` · ${stats.openCount} açık kayıt (detay: Risk Analizi)` : ""}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {viewMode === "timeline" && (
                  <div className="rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]">
                    <h3 className="text-sm font-semibold text-foreground">Tespit özeti</h3>
                    <p className="mt-2 text-xs text-muted-foreground leading-relaxed">
                      Bu oturumdaki tüm AI bulguları otomatik olarak Risk Analizi kaydına aktarılır. Zaman çizelgesi ve
                      tekrarlayan risk listeleri burada gösterilmez; tek inceleme ekranı Risk Analizi’dir.
                    </p>
                    {stats && stats.totalDetections > 0 ? (
                      <div className="mt-4 flex flex-wrap items-center gap-3">
                        <p className="text-sm text-foreground">
                          <strong>{stats.totalDetections}</strong> kayıt · Kritik {stats.criticalCount} · Yüksek{" "}
                          {stats.highCount} · Açık {stats.openCount}
                        </p>
                      ...
                      <p className="mt-4 py-6 text-center text-sm text-muted-foreground">Bu oturumda kayıtlı tespit yok.</p>
                    )}
                  </div>
                )}
              </>
