  ...
  try {
    ...
    if (!authContext) {
      ...
      const auth = allowReadOnlyLegalFallback || allowNavigationFallback || allowGreetingFallback || allowProductHelpFallback
        || allowGuidanceFallback || allowAuditSimulationFallback
      ...
      const hasNovaPermission = navigationIntent || greetingIntent || guidanceIntent || auditSimulationIntent || productHelpIntent
        ? true
        : await hasAiUsePermission(payload.access_token!);
      const hasManagerAccess = navigationIntent || greetingIntent || guidanceIntent || auditSimulationIntent || productHelpIntent
        ? true
