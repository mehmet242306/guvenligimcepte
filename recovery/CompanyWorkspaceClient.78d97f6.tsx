...
function buildMockDocuments(company: CompanyRecord) {
  return [
    ...
    {
      title: `${company.shortName || company.name} Acil Durum Plan─▒`,
      type: "Acil Durum",
      status: "Kontrol Gerekli",
    },
    {
      title: `${company.shortName || company.name} E─şitim Plan─▒`,
      type: "E─şitim",
      status: "Aktif",
    },
    {
...
function buildMockActivities(company: CompanyRecord) {
  return [
    {
      actor: "Mehmet Y─▒ld─▒r─▒m",
      role: "─░┼ş G├╝venli─şi Uzman─▒",
      action: `${company.shortName || company.name} i├ğin risk analizi g├Âzden ge├ğirildi.`,
      time: "Bug├╝n ┬À 14:20",
    },
    {
      actor: "Ay┼şe Demir",
      role: "─░┼şyeri Hekimi",
      action: "Sa─şl─▒k g├Âzetimi ve e─şitim plan─▒ notlar─▒ g├╝ncellendi.",
      time: "Bug├╝n ┬À 10:05",
    },
    {
      actor: "Ali Kaya",
      role: "─░┼şveren Vekili",
      action: "2 adet aksiyon i├ğin termin onay─▒ verildi.",
      time: "D├╝n ┬À 16:40",
    },
  ];
}
...
export function CompanyWorkspaceClient({
  ...
  function handleSave() {
    ...
    if (!company.name.trim()) {
      setMessage("Firma / kurum ad─▒ bo┼ş b─▒rak─▒lamaz.");
      setMessageType("error");
      return;
    }

    saveCompanyDirectory(companies);
    setMessage("─░┼şyeri ├ğal─▒┼şma alan─▒ bilgileri kaydedildi.");
    setMessageType("success");
  }

  const tabs: Array<{ id: WorkspaceTab; label: string }> = [
    { id: "overview", label: "Genel Durum" },
    { id: "structure", label: "Yerle┼şke / Yap─▒" },
    { id: "risk", label: "Risk ve Saha" },
    { id: "people", label: "Personel" },
    { id: "tracking", label: "Takip" },
    { id: "documents", label: "Dok├╝manlar" },
    { id: "history", label: "Ge├ğmi┼ş" },
    { id: "digital_twin", label: "Dijital ─░kiz" },
  ];

  if (!company) {
    return (
      <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.06)]">
        <h1 className="text-3xl font-semibold text-slate-950">Kay─▒t bulunamad─▒</h1>
        <p className="mt-3 text-sm leading-7 text-slate-600">
          ─░stenen firma / kurum kayd─▒ bulunamad─▒.
        </p>
        <Link
          href="/companies"
            ...
            <div className="max-w-4xl space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-3xl text-sm leading-8 text-slate-600 sm:text-base">
                  {company.notes ||
                    "Bu ekran i┼şyerinin ─░SG operasyon merkezi olarak kullan─▒l─▒r."}
                </p>
              </div>

              <div className="flex flex-wrap gap-2">
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600">
                  {company.kind || "T├╝r yok"}
                </span>
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600">
                  {company.hazardClass || "Tehlike s─▒n─▒f─▒ yok"}
                </span>
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600">
                  ├çal─▒┼şan: {company.employeeCount}
                </span>
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600">
                  Lokasyon: {company.locations.filter(Boolean).length}
                </span>
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600">
                  B├Âl├╝m: {company.departments.filter(Boolean).length}
                </span>
              </div>
            </div>
          ...
          <div className="border-b border-slate-200/70 p-5 md:border-r xl:border-b-0">
            ...
            <div className="mt-3 flex items-center gap-3">
              {riskState ? (
                ...
                >
                  {riskState.label}
                  {riskState.score !== null ? ` ┬À ${riskState.score}/100` : ""}
                </span>
              ) : null}
            </div>
          </div>
            ...
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm leading-7 text-slate-600">
                  Sistem bu i┼şyeri i├ğin ├Âncelikli i┼şleri ├Âne ├ğ─▒kar─▒r.
                </p>
              </div>
          ...
          <div className={sectionCardClass()}>
            ...
            {activeTab === "risk" ? (
              <div className="space-y-5">
                ...
                <div className="rounded-[24px] border border-slate-200 bg-slate-50/70 p-5">
                  <h3 className="text-base font-semibold text-slate-950">
                    Risk ve saha y├Ânetimi
                  </h3>
                  <p className="mt-3 text-sm leading-7 text-slate-600">
                    Risk analizi, saha tespiti, g├Ârsel y├╝kleme ve ileride canl─▒ saha taramas─▒ bu i┼şyeri ba─şlam─▒nda ├ğal─▒┼ş─▒r.
                  </p>
            ...
            {activeTab === "tracking" ? (
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                />
                <Input
                  label="Yenileme Yakla┼şan E─şitim"
                  type="number"
            ...
            {activeTab === "digital_twin" ? (
              <div className="space-y-4">
                <div className="rounded-[24px] border border-slate-200 bg-slate-50/70 p-5">
                  <h3 className="text-base font-semibold text-slate-950">
                    Dijital ikiz yakla┼ş─▒m─▒
                  </h3>
                  <p className="mt-3 text-sm leading-7 text-slate-600">
                    Bu i┼şyeri i├ğin dijital ikiz; lokasyon, b├Âl├╝m, risk analizi, saha taramas─▒, dok├╝man ve i┼şlem ge├ğmi┼şinin tek kurumsal haf─▒zada birle┼şmesiyle kurulacakt─▒r.
                  </p>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="rounded-[24px] border border-slate-200 bg-white p-4">
                    ...
                    <p className="mt-2 text-sm font-semibold text-slate-950">
                      Canl─▒ saha taramas─▒ + alan e┼şleme
                    </p>
                  </div>
