...
export function resolveExportImageSections(data: RiskAnalysisExportData): ExportImageSection[] {
  ...
  return data.images.map((img, i) => {
    ...
    return {
      ...
      isgKapsamindaMi: img.isgKapsamindaMi,
      scopeDecision: img.scopeDecision,
      scopeReason: img.scopeReason,
      containsWorkers: img.containsWorkers,
      containsWorkActivity: img.containsWorkActivity,
      containsWorkAtHeight: img.containsWorkAtHeight,
      containsOpenEdge: img.containsOpenEdge,
      containsScaffoldOrPlatform: img.containsScaffoldOrPlatform,
      containsLadder: img.containsLadder,
      containsRebar: img.containsRebar,
      containsMachinery: img.containsMachinery,
      containsPpeIssue: img.containsPpeIssue,
      dataUrl: img.dataUrl,
